...
class TestSessionBuilder extends AnyFlatSpec with Matchers with BeforeAndAfterEach {
  ...
  it should "handle mixed events and leave only unmatched docOpens" in {
    ...
    session.docOpens shouldBe Seq(doo1, doo2, doo3)
  }
  ...
  it should "handle multiple CardSearch and QuickSearch with doc opens" in {
    ...
    val cs1 = CardSearch(
      dateTime = Some(LocalDateTime.of(2023, 1, 1, 10, 0)),
      queriesTexts = Seq("query1"),
      searchResult = SearchResult("1CS1", Seq("DOC1"))
    )

    val cs2 = CardSearch(
      dateTime = Some(LocalDateTime.of(2023, 1, 1, 10, 5)),
      queriesTexts = Seq("query2"),
      searchResult = SearchResult("1CS2", Seq("DOC2"))
    )

    val qs1 = QuickSearch(
      dateTime = Some(LocalDateTime.of(2023, 1, 1, 10, 10)),
      queryText = "query3",
      searchResult = SearchResult("1QS1", Seq("DOC3"))
    )

    val doo1 = DocumentOpen(
      dateTime = Some(LocalDateTime.of(2023, 1, 1, 10, 15)),
      searchId = "1CS1",
      documentId = "DOC1"
    )

    val doo2 = DocumentOpen(
      dateTime = Some(LocalDateTime.of(2023, 1, 1, 10, 20)),
      searchId = "1CS2",
      documentId = "DOC2"
    )

    val doo3 = DocumentOpen(
      dateTime = Some(LocalDateTime.of(2023, 1, 1, 10, 25)),
      searchId = "1QS1",
      documentId = "DOC3"
    )
