...
class TestCountDocumentOpens extends AnyFlatSpec with Matchers with TestSparkContext {
    "countDocumentOpens" should "correctly count document opens after quick search" in {
        val session = Session(
            ...
            events = Seq(
                QuickSearchEvent(
                    timestamp = LocalDateTime.of(2023, 8, 1, 12, 0),
                    searchId = "s1",
                    queryText = "ACC_45616",
                    relatedDocuments = Seq("ACC_45616", "DOC_123")
                ),
                DocumentOpenEvent(
                    timestamp = LocalDateTime.of(2023, 8, 1, 12, 5),
                    searchId = "s1",
                    documentId = "ACC_45616"
                ),
                DocumentOpenEvent(
                    timestamp = LocalDateTime.of(2023, 8, 1, 12, 10),
                    searchId = "s1",
                    documentId = "ACC_45616"
                ),
                DocumentOpenEvent(
                    timestamp = LocalDateTime.of(2023, 8, 1, 12, 15),
                    searchId = "s2",
                    documentId = "ACC_45616"
                )
            )
        ...
        val result = Main.countDocumentOpens(sc.parallelize(Seq(session)))
    ...
    it should "not count document opens without matching quick search" in {
        val session = Session(
            sessionId = "session2",
            events = Seq(
                QuickSearchEvent(
    ...
    it should "count document opens across multiple sessions" in {
        ...
        val session2 = Session(
            ...
            events = Seq(
                QuickSearchEvent(
                    timestamp = LocalDateTime.of(2023, 8, 1, 13, 0),
                    searchId = "s2",
                    queryText = "ACC_45616",
                    relatedDocuments = Seq("ACC_45616")
                ),
                DocumentOpenEvent(
                    timestamp = LocalDateTime.of(2023, 8, 1, 13, 5),
                    searchId = "s2",
                    documentId = "ACC_45616"
                )
            )
        ...
        val result = Main.countDocumentOpens(sc.parallelize(Seq(session1, session2)))
    ...
    it should "count document opens on different dates and sessions separately" in {
        ...
        val session2 = Session(
            ...
            events = Seq(
                QuickSearchEvent(
                    timestamp = LocalDateTime.of(2023, 8, 2, 12, 0),
                    searchId = "s2",
                    queryText = "ACC_45616",
                    relatedDocuments = Seq("ACC_45616")
                ),
                DocumentOpenEvent(
                    timestamp = LocalDateTime.of(2023, 8, 2, 12, 5),
                    searchId = "s2",
                    documentId = "ACC_45616"
                )
            )
...
class TestCountSearchesForDocument extends AnyFlatSpec with Matchers with TestSparkContext {
    "countSearchesForDocument" should "count only matching CardSearchEvents in multiple sessions" in {
        ...
        val session2 = Session(
            ...
            events = Seq(
                CardSearchEvent(
                    timestamp = LocalDateTime.of(2023, 8, 2, 10, 0),
                    searchId = "cs3",
        ...
        val result = Main.countSearchesForDocument(sc.parallelize(Seq(session1, session2)), "ACC_45616")
    ...
    it should "return zero if no matching documents found" in {
        ...
        val result = Main.countSearchesForDocument(sc.parallelize(Seq(session)), "ACC_45616")
        result shouldBe 0
    }
